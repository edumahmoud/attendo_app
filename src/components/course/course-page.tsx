...
export default function CoursePage({ profile, role }: CoursePageProps) {
  const { selectedSubjectId, courseTab, setSelectedSubjectId, setCourseTab, openProfile, courseChatUnread } = useAppStore();
          ...
          {availableTabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {tab.id === 'chat' && courseChatUnread > 0 && (
                  <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 text-white text-[9px] font-bold px-1 leading-none">
                    {courseChatUnread > 99 ? '99+' : courseChatUnread}
                  </span>
                )}
              </button>
            );
