...
export default function ChatTab({ profile, role, subjectId, subject }: ChatTabProps) {
  ...
  const { setCourseChatUnread } = useAppStore();
  // State
  ...
  const initConversation = useCallback(async () => {
    ...
    try {
      ...
      if (data.conversation?.id) {
        ...
        try {
          const unreadRes = await fetch(`/api/chat?action=conversations&userId=${profile.id}`);
          const unreadData = await unreadRes.json();
          const thisConv = (unreadData.conversations || []).find((c: { id: string }) => c.id === convId);
          const count = thisConv?.unreadCount || 0;
          setUnreadCount(count);
          setCourseChatUnread(count);
        } catch { /* ignore */ }
      } else {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(pollMessages, isConnected ? 30000 : 5000);
  ...
  useSocketEvent<ChatMessage>('new-message', (msg) => {
    ...
    // Deduplicate: skip if already processed via chat-notification
    ...
    if (msgConvId === currentConvId) {
      setMessages((prev) => {
        ...
        // Check for optimistic duplicate (own message echoed back)
      ...
      if (senderId !== profile.id && currentConvId) {
        fetch('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  useSocketEvent<{
    ...
    // Deduplicate: if new-message already processed this, skip
  ...
  const renderMessage = (msg: ChatMessage, index: number) => {
    ...
    return (
      ...
      >
        {/* Avatar - shown above-left of the message */}
        {!isOwn && (
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 mt-3 flex items-end gap-2 bg-background pb-1">
        <div className="flex-1 relative">
          <input
            ...
            className="w-full rounded-xl border bg-background px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all"
            dir="rtl"
            disabled={sending}
          />
        </div>
