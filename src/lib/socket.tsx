...
const SOCKET_URL = (() => {
  ...
  // If behind Caddy proxy (port 81 or default 80/443), use proxy path
  if (port === '81' || port === '' || port === '80' || port === '443') {
    return '/?XTransformPort=3003';
  }
  // Otherwise (direct Next.js dev on port 3000) connect directly to chat service
...
export function SocketProvider({ children }: SocketProviderProps): JSX.Element {
  ...
  // ─── Ensure chat service is running on mount ───
  useEffect(() => {
    fetch('/api/chat/ensure-service').catch(() => {
      // Silently ignore — service might not be available
    });
  }, []);
