-- =====================================================
-- ATTENDO - COMPLETE SUPABASE SCHEMA (الإصدار الكامل)
-- المنصة التعليمية الذكية - منصة أتيندو
-- =====================================================
-- ⚠️  هذا الملف يمسح كل البيانات الموجودة وينشئ قاعدة بيانات جديدة
...
-- =====================================================
-- PART 0: CLEAN SLATE - حذف كل الجداول الموجودة
-- ⚠️  احذف هذا الجزء لو عايز تحافظ على البيانات الموجودة!
-- =====================================================
...
-- =====================================================
-- PART 1: USERS (لا يعتمد على أي جدول - يجب أن يكون الأول)
-- =====================================================
...
-- =====================================================
-- PART 2: TEACHER-STUDENT LINKS (يعتمد على: users)
-- =====================================================
...
CREATE INDEX idx_tsl_status ON public.teacher_student_links(teacher_id, status);

-- =====================================================
-- PART 3: SUMMARIES (يعتمد على: users)
-- =====================================================
...
-- =====================================================
-- PART 4b: SUBJECT_TEACHERS (يعتمد على: subjects, users)
-- =====================================================
...
-- =====================================================
-- PART 5: SUBJECT_STUDENTS (يعتمد على: subjects, users)
-- =====================================================
...
CREATE INDEX idx_subject_students_status ON public.subject_students(subject_id, status);

-- =====================================================
-- PART 6: LECTURES (يعتمد على: subjects)
-- =====================================================
...
-- =====================================================
-- PART 7: QUIZZES (يعتمد على: users, summaries, subjects)
-- =====================================================
...
-- =====================================================
-- PART 8: SCORES (يعتمد على: users, quizzes)
-- =====================================================
...
-- =====================================================
-- PART 9: LECTURE_NOTES (يعتمد على: lectures, users)
-- =====================================================
...
-- =====================================================
-- PART 10: NOTE_VIEWS (يعتمد على: lecture_notes, users)
-- =====================================================
...
-- =====================================================
-- PART 11: ASSIGNMENTS (يعتمد على: subjects, users)
-- =====================================================
...
-- =====================================================
-- PART 12: USER_FILES (يعتمد على: users, assignments)
-- =====================================================
...
CREATE INDEX idx_user_files_visibility ON public.user_files(visibility);

-- =====================================================
-- PART 13: FILE_SHARES (يعتمد على: user_files, users)
-- =====================================================
...
-- =====================================================
-- PART 14: SUBJECT_FILES (يعتمد على: subjects, users, user_files)
-- =====================================================
...
CREATE TABLE public.subject_files (
  ...
  user_file_id UUID REFERENCES public.user_files(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ DEFAULT now() NOT NULL
);
...
CREATE INDEX idx_subject_files_visibility ON public.subject_files(visibility);
CREATE INDEX idx_subject_files_user_file_id ON public.subject_files(user_file_id);

-- =====================================================
-- PART 15: FILE_REQUESTS (يعتمد على: user_files, users)
-- =====================================================
...
CREATE INDEX idx_file_requests_file_id ON public.file_requests(file_id);
CREATE INDEX idx_file_requests_requester_id ON public.file_requests(requester_id);
CREATE INDEX idx_file_requests_owner_id ON public.file_requests(owner_id);
CREATE INDEX idx_file_requests_status ON public.file_requests(status);

-- =====================================================
-- PART 16: SUBMISSIONS (يعتمد على: assignments, users, user_files)
-- =====================================================
...
-- =====================================================
-- PART 17: ATTENDANCE_SESSIONS (يعتمد على: lectures, users, subjects)
-- =====================================================
...
-- =====================================================
-- PART 18: ATTENDANCE_RECORDS (يعتمد على: attendance_sessions, users)
-- =====================================================
...
CREATE TABLE public.attendance_records (
  ...
  check_in_method TEXT CHECK (check_in_method IN ('qr', 'gps', 'manual')),
...
-- =====================================================
-- PART 19: NOTIFICATIONS (يعتمد على: users)
-- =====================================================
...
-- =====================================================
-- PART 20: USER_SESSIONS (يعتمد على: users)
-- =====================================================
...
-- =====================================================
-- PART 21: ANNOUNCEMENTS (يعتمد على: users)
-- =====================================================
...
-- =====================================================
-- PART 24: CONVERSATIONS (نظام المحادثات)
-- =====================================================
...
-- =====================================================
-- PART 25: CONVERSATION_PARTICIPANTS (يعتمد على: conversations, users)
-- =====================================================
...
-- =====================================================
-- PART 26: MESSAGES (يعتمد على: conversations, users)
-- =====================================================
...
CREATE POLICY "Teachers can delete links" ON public.teacher_student_links
...
CREATE POLICY "Teachers can read linked student summaries" ON public.summaries
  FOR SELECT USING (
    user_id IN (SELECT student_id FROM public.teacher_student_links WHERE teacher_id = auth.uid() AND status = 'approved')
  );
...
-- ===== SUBJECT_STUDENTS (مع نظام القبول/الرفض) =====
...
CREATE POLICY "Students can view own enrollments" ON public.subject_students
  FOR SELECT USING (student_id = auth.uid());
CREATE POLICY "Insert enrollment" ON public.subject_students
  FOR INSERT WITH CHECK (
    -- Teachers can insert with any status
    subject_id IN (SELECT public.get_teacher_subject_ids(auth.uid()))
    OR
    -- Students can only insert with status='pending' or default
    (student_id = auth.uid())
  );
CREATE POLICY "Teachers can manage enrollment status" ON public.subject_students
  FOR UPDATE USING (
    subject_id IN (SELECT public.get_teacher_subject_ids(auth.uid()))
  );
CREATE POLICY "Students can delete own enrollments" ON public.subject_students
  FOR DELETE USING (student_id = auth.uid());
...
CREATE POLICY "Students can read teacher quizzes" ON public.quizzes
  FOR SELECT USING (
    user_id IN (SELECT teacher_id FROM public.teacher_student_links WHERE student_id = auth.uid() AND status = 'approved')
  );
...
-- ===== FILE_REQUESTS =====
CREATE POLICY "Users can view own file requests" ON public.file_requests
...
CREATE POLICY "File owners can update request status" ON public.file_requests
...
CREATE POLICY "Students can check in to attendance" ON public.attendance_records
  FOR INSERT WITH CHECK (student_id = auth.uid());
CREATE POLICY "Teachers can insert attendance for own sessions" ON public.attendance_records
  FOR INSERT WITH CHECK (
    session_id IN (SELECT id FROM public.attendance_sessions WHERE teacher_id = auth.uid())
  );
...
-- =====================================================
-- PART 30: الدوال المساعدة (SECURITY DEFINER - لمنع التكرار اللانهائي في RLS)
-- =====================================================
...
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.subject_students WHERE subject_students.subject_id = subject_id AND subject_students.student_id = student_id AND status = 'approved');
$$;
...
RETURNS SETOF UUID LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT subject_id FROM public.subject_students WHERE subject_students.student_id = student_id AND status = 'approved';
$$;
...
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    ...
    WHERE l.id = lecture_id AND ss.student_id = student_id AND ss.status = 'approved'
  );
$$;
...
-- =====================================================
-- PART 31: الدوال والتريجرز
-- =====================================================

-- توليد كود المعلم تلقائياً
...
-- إنشاء بروفايل المستخدم تلقائياً عند التسجيل
...
-- إضافة صاحب المقرر تلقائياً في subject_teachers
...
-- دالة إعداد المؤسسة (للـ Setup Wizard)
...
-- إنشاء محادثة جماعية تلقائياً عند إنشاء مقرر
...
-- إضافة الطالب للمحادثة الجماعية عند قبوله في المقرر
...
-- إضافة المعلم للمحادثة الجماعية
...
CREATE POLICY "Students can read subject files" ON storage.objects
  FOR SELECT USING (
    ...
    EXISTS (
      ...
      WHERE ss.student_id = auth.uid() AND ss.status = 'approved' AND sf.file_url::text LIKE '%' || name::text || '%'
    )
  );
